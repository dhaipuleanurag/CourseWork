...
\[
\frac{\partial J(\theta)}{\partial \theta _k}=\frac{1}{m}\sum_{i=1}^{m}\left(h_{\theta}(x_{i})-y_{i}\right)x_{i}^{k},
\]
In matrix form
\[
\del_{\theta}J(\theta)=\frac{1}{m}(X\theta - y)^{T}X,
\]
...
The equation of a line with point $ (x_{1}, y_{1})$ and slope m
\[
(y -  y_{1} ) = m(x - x_{1}) 
\]
Here slope is given by $\frac{\partial J( \theta)}{\partial \theta}$
Substituting values in line question we get
\[
J(\theta+\eta\Delta)-J(\theta) = \frac{\partial J( \theta)}{\partial \theta _k} (\eta \Delta)
\]

The value of $\frac{\partial J( \theta)}{\partial \theta _k}$ is derived in part 2 above.

\item Write down the expression for updating $\theta$ in the gradient descent
algorithm. Let $\eta$ be the step size. 

\[
\theta _k = \theta _k - \eta \frac{\partial J( \theta)}{\partial \theta _k}
\]
...
$X= ([1,2,3,1],[0,1,7,1])$ $\theta = ([3,4,1,2])$ and  $y = ([20,25])$
...
Considering the same values for X, $\theta$ and y as in 2.2.6, we get
\[
\del_{\theta}J(\theta) = [-2, -10, -48, -8]
\]
...
With step size = 0.5, the initial loss was close to 441.4 and it went higher and higher, which means the step size taken is very large. The next $\theta$ value we taking is going beyond the minimum point. This diverges.
...
\includegraphics[width=\textwidth]{figure_1_batchgradient.png}
...
\[
\del_{\theta}J(\theta)=\frac{1}{m}(X\theta - y)^{T}X + 2 \lambda \theta,
\]

The update rule for $\theta$ can be written as 
\[
\theta _k = \theta _k - \eta \frac{\partial J( \theta)}{\partial \theta _k}
\]
In matrix form
\[
\theta = \theta - \eta \del_{\theta}J(\theta)
\]
...
The idea of regularization is to avoid the problem of over fitting. To avoid over fitting we need to make the curve smoother. This can be achieved by reducing the magnitude of $\theta$ excluding the bias term.
...
\textbf{Things to be noted here is that validation loss decreases with $\lambda$ value and then increases.}

\includegraphics[width = \textwidth]{figure_2_trainingvalidationloss.png}
...
We found that $\lambda$ = 10 and step size = 0.01 produced less validation loss. Hence corresponding $\theta$ obtained by calling \texttt{regularized\_grad\_descent} is used.
We used validation loss to determine $\theta$ because our function is not trained using the test data and hence best results with test data is used for determining best $\theta$

Value of $\theta$ used is
 
 [ -9.92635627e-03   6.67543885e-05   6.40706144e-03   8.43709881e-03
  -1.42228569e-03  -1.34379919e-04   1.32553192e-03   1.32553192e-03
   7.26646711e-03   1.27152013e-02   1.42641663e-02   6.47452075e-03
  -2.25259582e-03  -7.04382209e-03   1.37839141e-02   1.64218885e-02
   1.33508992e-02   2.75471484e-03   6.56536406e-04   6.56536406e-04
   6.56536406e-04   2.69293752e-03   2.69293752e-03   2.69293752e-03
   3.27840986e-03   3.27840986e-03   3.27840986e-03   3.54451035e-03
   3.54451035e-03   3.54451035e-03   3.69052930e-03   3.69052930e-03
   3.69052930e-03   5.25843606e-03   5.25843606e-03   5.25843606e-03
   5.78340748e-03   5.78340748e-03   5.78340748e-03   5.05211594e-03
   5.05211594e-03   5.05211594e-03   4.71122269e-03   4.71122269e-03
   4.71122269e-03   4.52134806e-03   4.52134806e-03   4.52134806e-03
  -1.28522936e-02]
...
\[
J(\theta)=\frac{1}{2}(x _{k}\theta - y _{k})^{T}(x _{k}\theta - y _{k}) + \lambda\theta^{T}\theta,
\]

Where $x _{k}$ and $y _{k}$ is one the sample choosen randomly from the all the input samples.

Differentiating

\[
\del_{\theta}J(\theta)=(x _{k}\theta - y _{k})x _{k} + 2 \lambda \theta,
\]
...
Graph for loss with respect to epochs is shown below for step size = 1/t and step size = 1/sqrt(t). Note that, for step size = 1/t, the graph converges slowly compared to step size = 1/sqrt(t). This is because with step size = 1/t, the step size reduces quickly. And lower value of step size implies slower rate of convergence.
Also to be noted is that with SGD the graph converges much faster compared to GD.

\includegraphics[width=\textwidth]{figure_3_sgd.png}
...
\includegraphics[width=\textwidth]{figure_4_sgdvsgd}
\end{enumerate}
...
\begin{enumerate}
\item Show that for the square loss $\ell(\hat{y},y)=\frac{1}{2}\left(y-\hat{y}\right)^{2}$,the Bayes decision function is a $f^{*}(x)=\ex\left[Y\mid X=x\right]$.
{[}Hint: Consider constructing $f^{*}(x)$, one $x$ at a time.{]}

Square loss function can be written as

\[
\ell(f(x),y)=\frac{1}{2}\left(f(x)-y\right)^{2},
\]

The risk of a square loss function will be 

\[
R(f) = \ex \frac{1}{2}\left(f(x)-y\right)^{2},
\]

\[
R(f) = \ex \frac{1}{2}(f(x)^{2} + y^{2} - 2 f(x) y)
\]
